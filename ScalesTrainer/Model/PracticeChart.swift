...
class ScaleToChart: ObservableObject, Codable, Identifiable, Hashable {
    static func == (lhs: ScaleToChart, rhs: ScaleToChart) -> Bool {
        lhs.id == rhs.id
    }
    ...
    init(scaleId: String, practiceDay: Int) {
        self.id = UUID()
...
class PracticeChart: Codable {
    ...
    init(user:User, board:String, grade:Int, minorScaleType:Int = 0) {
        ...
        //shuffle()
        for scale in scales {
            self.chartScales.append(ScaleToChart(scaleId: scale.getScaleIdentificationKey(), practiceDay: 0))
        }
        shuffle()
    }
    ...
    func debug11(_ ctx:String) {
        for r in 0..<self.chartScales.count {
            print("cell \(self.chartScales[r].scaleId)", "day", self.chartScales[r].practiceDay)
        }
    }
    ...
    func shuffle() {
        var indexes = Array(chartScales.indices)
        indexes.shuffle()
        var dayNumber = 0
        for i in indexes  {
            chartScales[i].practiceDay = dayNumber
            chartScales[i].republish()
